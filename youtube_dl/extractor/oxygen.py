...
class OxygenTVIE(AdobePassIE):
    ...
    def _real_extract(self, url):
        ...
        season_number = int_or_none(self._search_regex(
            r'<div class="video__label">.*?S(\d+)', webpage, 'season number',
            default=None, flags=re.DOTALL))

        episode_number = int_or_none(self._search_regex(
            r'<div class="video__label">.*?E(\d+)', webpage, 'episode number',
            default=None, flags=re.DOTALL))

        series = self._search_regex(
            r's\.contextData\["nbcu\.showsite\"\]="(.+)"', webpage, 'Series',
            default=None)
